...
-   Prior $\b \mid \phi \sim \N(\zero_b, \frac{1} {\phi \kappa} \I_p$)
    implies the $\bv$ are exchangable *a priori* (i.e. distribution is
...
-   Equivaletnt to finding posterior mode $$
...
-   Issue is that the tails of the prior under the double exponential
    are not heavier than the the normal likelihood
...
\I \b + \eps$ 
$$
E[\beta_i \mid \Y] = \int_0^1 (1 - \kappa_i) y^*_i p(\kappa_i \mid \Y)\ d\kappa_i = (1 - \E[\kappa \mid y^*_i]) y^*_i$$


- $\kappa_i = 1/(1 + \tau_i^2)$ shrinkage factor 

- Posterior mean
$E[\beta \mid y] = y + \frac{d} {d y} \log m(y)$
where $m(y)$ is the predictive density under the prior (known $\lambda$) 
 
- HS has Bounded Influence: if $\lim_{|y| \to \infty} \frac{d}{dy} \log m(y) = 0$ 
$$\lim_{|y| \to \infty} E[\beta \mid y) \to y $$
- DE prior also has bounded influence, but bound does not decay to zero in tails
...
::: {.callout-warning} 
## Homework
Check the conditions for the Generalized Double Pareto 
:::
...
- If selection is a goal of analysis build it into the model/analysis
